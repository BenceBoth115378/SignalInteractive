...
def _state_snapshot_rows(state: dict) -> list[tuple[str, str, str | None, Any]]:
    ...
    return [
        ("Alice Generated", str(bool(state.get("alice_generated", False))), _tt("x3dh_step_state_alice_generated"), state.get("alice_generated", False)),
        ("Alice Bundle Uploaded", str(isinstance(server.get("alice_bundle"), dict)), _tt("x3dh_step_state_alice_bundle"), server.get("alice_bundle")),
        ("Alice OPK on Server", str(_opk_count(server, "alice_available_opk_ids")), _tt("x3dh_step_state_alice_opk"), server.get("alice_available_opk_ids")),
        ("Bob OPK on Server", str(_opk_count(server, "bob_available_opk_ids")), _tt("x3dh_step_state_bob_opk"), server.get("bob_available_opk_ids")),
        ("Bundle For Alice", str(isinstance(state.get("last_bundle_for_alice"), dict)), _tt("x3dh_step_state_bundle_for_alice"), state.get("last_bundle_for_alice")),
        ("Signature Verified", str(bool(state.get("phase2_signature_verified", False))), _tt("x3dh_step_state_signature_verified"), state.get("phase2_signature_verified", False)),
        ("EK/SK Derived", str(bool(state.get("phase2_ek_generated", False))), _tt("x3dh_step_state_ek_sk"), state.get("phase2_ek_generated", False)),
        ("AD Computed", str(isinstance(derived, dict) and isinstance(derived.get("associated_data"), str)), _tt("x3dh_step_state_ad"), derived.get("associated_data") if derived else None),
        ("Initial Message", str(isinstance(state.get("initial_message"), dict)), _tt("x3dh_step_state_initial_message"), state.get("initial_message")),
        ("Bob Verification", str(isinstance(state.get("bob_receive_result"), dict)), _tt("x3dh_step_state_bob_verification"), state.get("bob_receive_result")),
        ("Events", str(len(state.get("events", []))), _tt("x3dh_step_state_events"), state.get("events", [])),
    ]
...
def _alice_local_rows(payload: dict | None) -> list[tuple[str, str, str | None, Any]]:
    ...
    return [
        ("IK_pub", shared_steps.last_key_chars(identity.get("public", "-")), _tt("x3dh_step_key_ik_pub"), identity.get("public")),
        ("IK_priv", shared_steps.last_key_chars(identity.get("private", "-")), _tt("x3dh_step_key_ik_priv"), identity.get("private")),
        ("SPK_pub", shared_steps.last_key_chars(spk.get("public", "-")), _tt("x3dh_step_key_spk_pub"), spk.get("public")),
        ("SPK_priv", shared_steps.last_key_chars(spk.get("private", "-")), _tt("x3dh_step_key_spk_priv"), spk.get("private")),
        ("SPK_sig", shared_steps.last_key_chars(payload.get("signed_prekey_signature", "-")), _tt("x3dh_step_key_spk_sig"), payload.get("signed_prekey_signature")),
        ("OPK_count_local", str(len(opk_map)), None, sorted(opk_map.keys())),
    ]


def _server_alice_rows(server: dict | None) -> list[tuple[str, str, str | None, Any]]:
    ...
    return [
        ...
        ("IK_pub(server)", shared_steps.last_key_chars(bundle.get("identity_dh_public", "-") if bundle else "-"), _tt("x3dh_step_key_ik_pub"), bundle.get("identity_dh_public") if bundle else None),
        ("SPK_pub(server)", shared_steps.last_key_chars(bundle.get("signed_prekey_public", "-") if bundle else "-"), _tt("x3dh_step_key_spk_pub"), bundle.get("signed_prekey_public") if bundle else None),
        ("SPK_sig(server)", shared_steps.last_key_chars(bundle.get("signed_prekey_signature", "-") if bundle else "-"), _tt("x3dh_step_key_spk_sig"), bundle.get("signed_prekey_signature") if bundle else None),
        ("OPK_count_server", str(len(opk_ids)), None, opk_ids),
        ("OPK_ids_server", ", ".join(str(item) for item in opk_ids[:8]) or "-", None, opk_map),
    ]


def _server_bob_rows(server: dict | None) -> list[tuple[str, str, str | None, Any]]:
    if not isinstance(server, dict):
        return [("Server", "Unavailable", None, None)]
    ...
    return [
        ...
        ("IK_B_pub(server)", shared_steps.last_key_chars(bundle.get("identity_dh_public", "-") if bundle else "-"), _tt("x3dh_step_key_ik_pub"), bundle.get("identity_dh_public") if bundle else None),
        ("SPK_B_pub(server)", shared_steps.last_key_chars(bundle.get("signed_prekey_public", "-") if bundle else "-"), _tt("x3dh_step_key_spk_pub"), bundle.get("signed_prekey_public") if bundle else None),
        ("SPK_B_sig(server)", shared_steps.last_key_chars(bundle.get("signed_prekey_signature", "-") if bundle else "-"), _tt("x3dh_step_key_spk_sig"), bundle.get("signed_prekey_signature") if bundle else None),
        ("Bob_OPK_count_server", str(len(opk_ids)), None, opk_ids),
        ("Bob_OPK_ids_server", ", ".join(str(item) for item in opk_ids[:8]) or "-", None, opk_map),
    ]


def _bundle_for_alice_rows(bundle: dict | None) -> list[tuple[str, str, str | None, Any]]:
    if not isinstance(bundle, dict):
        return [("Bundle for Alice", "Not requested", None, None)]

    return [
        ("IK_B_pub", shared_steps.last_key_chars(bundle.get("identity_dh_public", "-")), _tt("x3dh_step_key_ik_pub"), bundle.get("identity_dh_public")),
        ("SPK_B_pub", shared_steps.last_key_chars(bundle.get("signed_prekey_public", "-")), _tt("x3dh_step_key_spk_pub"), bundle.get("signed_prekey_public")),
        ("SPK_B_sig", shared_steps.last_key_chars(bundle.get("signed_prekey_signature", "-")), _tt("x3dh_step_key_spk_sig"), bundle.get("signed_prekey_signature")),
        ("OPK_B_id", str(bundle.get("opk_id", "-")), None, bundle.get("opk_id")),
        ("OPK_B_pub", shared_steps.last_key_chars(bundle.get("opk_public", "-")), _tt("x3dh_step_key_opk_pub"), bundle.get("opk_public")),
    ]


def _alice_derived_rows(derived: dict | None) -> list[tuple[str, str, str | None, Any]]:
    if not isinstance(derived, dict):
        return [("Alice derived", "Not available", None, None)]

    return [
        ("EK_pub", shared_steps.last_key_chars(derived.get("ek_public", "-")), _tt("x3dh_step_key_ek_pub"), derived.get("ek_public")),
        ("EK_priv", shared_steps.last_key_chars(derived.get("ek_private", "-")), _tt("x3dh_step_key_ek_priv"), derived.get("ek_private")),
        ("SK", shared_steps.last_key_chars(derived.get("shared_secret", "-")), _tt("x3dh_step_key_sk"), derived.get("shared_secret")),
        ("AD", shared_steps.last_key_chars(derived.get("associated_data", "-")), _tt("x3dh_step_key_ad"), derived.get("associated_data")),
        ("DH_count", str(derived.get("dh_count", "-")), None, derived.get("dh_count")),
    ]


def _initial_message_rows(message: dict | None) -> list[tuple[str, str, str | None, Any]]:
    ...
    return [
        ("IK_A_pub", shared_steps.last_key_chars(ik_a_public), _tt("x3dh_step_key_ik_pub"), ik_a_public),
        ("EK_A_pub", shared_steps.last_key_chars(ek_a_public), _tt("x3dh_step_key_ek_pub"), ek_a_public),
        ("Bob SPK_pub", shared_steps.last_key_chars(bob_spk_public), _tt("x3dh_step_key_spk_pub"), bob_spk_public),
        ("Bob OPK id", str(bob_opk_id if bob_opk_id is not None else "-"), None, bob_opk_id),
        ("Ciphertext", shared_steps.last_key_chars(message.get("ciphertext", "-")), None, message.get("ciphertext")),
    ]


def _bob_receive_result_rows(result: dict | None) -> list[tuple[str, str, str | None, Any]]:
    if not isinstance(result, dict):
        return [("Bob result", "Not available", None, None)]

    return [
        ("Used OPK id", str(result.get("used_opk_id", "-")), None, result.get("used_opk_id")),
        ("AD_local", shared_steps.last_key_chars(result.get("ad_local", "-")), _tt("x3dh_step_key_ad"), result.get("ad_local")),
        ...
        ("Bob SK", shared_steps.last_key_chars(result.get("bob_shared_secret", "-")), _tt("x3dh_step_key_sk"), result.get("bob_shared_secret")),
        ...
        ("Decrypted_payload", shared_steps.preview(result.get("decrypted_text", "-"), 64), None, result.get("decrypted_text")),
...
def _build_request_bob_bundle_steps(before_state: dict, after_state: dict) -> list[dict[str, Any]]:
    ...
    step1 = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    shared_steps.state_panel("Server Bob state (before)", _server_bob_rows(before_server)),
                ],
        ...
        controls=[
            ...
            ft.Row(
                controls=[
                    ...
                    shared_steps.var_node("Output: Consumed OPK id", str(opk_id if opk_id is not None else "-"), width=280, height=90, full_value=opk_id),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=16,
                wrap=True,
    ...
    step3 = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    shared_steps.state_panel("Server Bob state (after)", _server_bob_rows(after_server), highlight_labels={"Bob_OPK_count_server", "Bob_OPK_ids_server"}),
                    shared_steps.state_panel("Alice bundle cache (after)", _bundle_for_alice_rows(after_bundle)),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=20,
                wrap=True,
...
def _build_verify_signature_steps(before_state: dict, after_state: dict) -> list[dict[str, Any]]:
    ...
    step1 = ft.Column(
        controls=[
            ...
            shared_steps.state_panel("Bundle for Alice", _bundle_for_alice_rows(bundle)),
        ],
        spacing=8,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    )
    ...
    step2 = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    shared_steps.var_node("IK_B_pub", shared_steps.last_key_chars(ik_pub), width=240, full_value=ik_pub, tooltip=_tt("x3dh_step_key_ik_pub")),
                    shared_steps.var_node("SPK_B_pub", shared_steps.last_key_chars(spk_pub), width=240, full_value=spk_pub, tooltip=_tt("x3dh_step_key_spk_pub")),
                    shared_steps.var_node("SPK_B_sig", shared_steps.last_key_chars(spk_sig), width=240, full_value=spk_sig, tooltip=_tt("x3dh_step_key_spk_sig")),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=12,
                wrap=True,
            ...
            ft.Text("↓", size=24),
            shared_steps.func_node("VERIFY", width=180, tooltip=_tt("x3dh_step_node_verify")),
            ft.Text("↓", size=24),
            shared_steps.var_node("is_valid", str(verify_result), width=260, full_value=verify_result),
            ft.Text(
...
def _build_generate_ek_and_sk_steps(before_state: dict, after_state: dict) -> list[dict[str, Any]]:
    ...
    step1 = ft.Column(
        controls=[
            ...
            shared_steps.state_panel("Bundle inputs", _bundle_for_alice_rows(bundle)),
        ],
        spacing=8,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    )
    ...
    dh_controls: list[ft.Control] = [
        shared_steps.var_node("DH1", "DH(IK_A_priv, SPK_B_pub)", width=420, full_value=f"IK_A_priv={ik_priv}\nSPK_B_pub={spk_b_pub}", tooltip=_tt("x3dh_step_node_dh")),
        shared_steps.var_node("DH2", "DH(EK_A_priv, IK_B_pub)", width=420, full_value=f"EK_A_priv={ek_priv}\nIK_B_pub={ik_b_pub}", tooltip=_tt("x3dh_step_node_dh")),
        shared_steps.var_node("DH3", "DH(EK_A_priv, SPK_B_pub)", width=420, full_value=f"EK_A_priv={ek_priv}\nSPK_B_pub={spk_b_pub}", tooltip=_tt("x3dh_step_node_dh")),
    ]
    if opk_b_pub not in {None, "", "-"}:
        dh_controls.append(shared_steps.var_node("DH4", "DH(EK_A_priv, OPK_B_pub)", width=420, full_value=opk_b_pub, tooltip=_tt("x3dh_step_node_dh")))
    ...
    step4 = ft.Column(
        controls=[
            ...
            shared_steps.state_panel("Alice derived (after)", _alice_derived_rows(after_state.get("alice_derived")), highlight_labels={"EK_pub", "EK_priv", "SK", "DH_count"}),
        ],
        spacing=8,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    )
...
def _build_calculate_ad_steps(before_state: dict, after_state: dict) -> list[dict[str, Any]]:
    alice = after_state.get("alice_local") if isinstance(after_state.get("alice_local"), dict) else {}
    bundle = after_state.get("last_bundle_for_alice") if isinstance(after_state.get("last_bundle_for_alice"), dict) else {}
    derived = after_state.get("alice_derived") if isinstance(after_state.get("alice_derived"), dict) else {}
...
def _build_send_initial_message_steps(before_state: dict, after_state: dict, action_context: dict[str, Any]) -> list[dict[str, Any]]:
    _ = action_context
    derived = after_state.get("alice_derived") if isinstance(after_state.get("alice_derived"), dict) else {}
        ...
        controls=[
            ...
            ft.Row(
                controls=[
                    ...
                    shared_steps.var_node("Bob_OPK_id", str(bob_opk_id), width=220, full_value=bob_opk_id, tooltip=_tt("x3dh_step_key_bob_opk_id")),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=12,
                wrap=True,
    ...
    step3 = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    shared_steps.var_node("Header", "IK_A_pub | EK_A_pub | Bob_SPK_pub | Bob_OPK_id",
                               width=300, full_value=header, tooltip=_tt("x3dh_step_node_header")),
                    shared_steps.var_node("Ciphertext", shared_steps.last_key_chars(ciphertext), width=300, full_value=ciphertext, tooltip=_tt("x3dh_step_key_ciphertext")),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=16,
                wrap=True,
...
def _build_bob_receives_steps(before_state: dict, after_state: dict) -> list[dict[str, Any]]:
    ...
    step1 = ft.Column(
        controls=[
            ...
            shared_steps.state_panel("Initial message", _initial_message_rows(msg)),
        ],
        spacing=8,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    )
    ...
    dh_controls: list[ft.Control] = [
        shared_steps.var_node("DH1", "DH(SPK_B_priv, IK_A_pub)", width=420, tooltip=_tt("x3dh_step_node_dh")),
        shared_steps.var_node("DH2", "DH(IK_B_priv, EK_A_pub)", width=420, tooltip=_tt("x3dh_step_node_dh")),
        shared_steps.var_node("DH3", "DH(SPK_B_priv, EK_A_pub)", width=420, tooltip=_tt("x3dh_step_node_dh")),
    ]
    if opk_used is not None:
        dh_controls.append(shared_steps.var_node("DH4", "DH(OPK_B_priv, EK_A_pub)", width=420, tooltip=_tt("x3dh_step_node_dh")))
    ...
    step3 = shared_steps.build_decrypt_step(
        ...
        key_tooltip=_tt("x3dh_step_key_sk"),
        ciphertext_tooltip=_tt("x3dh_step_key_ciphertext"),
        plaintext_tooltip=_tt("x3dh_step_key_ad"),
        decrypt_tooltip=_tt("x3dh_step_node_decrypt"),
    )
    ...
    step4 = ft.Column(
        controls=[
            ...
            ft.Text("↓", size=24),
            shared_steps.func_node("CALC_AD", "Compute AD from header identities", width=420, tooltip=_tt("x3dh_step_node_calc_ad")),
            ft.Text("↓", size=24),
    ...
    step5 = shared_steps.build_compare_values_step(
        ...
        left_tooltip=_tt("x3dh_step_key_ad"),
    )
...
def _build_generate_alice_keys_steps(before_state: dict, after_state: dict) -> list[dict[str, Any]]:
    ...
    return shared_steps.build_generate_alice_core_steps(
        before_alice_rows=_alice_local_rows(before_alice),
        before_server_rows=_server_alice_rows(before_state.get("server_state")),
...
def _build_upload_initial_bundle_steps(before_state: dict, after_state: dict) -> list[dict[str, Any]]:
    before_server = before_state.get("server_state") if isinstance(before_state.get("server_state"), dict) else {}
    after_server = after_state.get("server_state") if isinstance(after_state.get("server_state"), dict) else {}
    ...
    step1 = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    shared_steps.state_panel("Alice local (before)", _alice_local_rows(before_state.get("alice_local"))),
                    shared_steps.state_panel("Server (before)", _server_alice_rows(before_server)),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=20,
                wrap=True,
    ...
    step2 = ft.Column(
        controls=[
            ...
            shared_steps.var_node("Alice local state", "IK + SPK + signature + OPKs", width=300, height=95),
            ft.Text("↓", size=24),
            shared_steps.func_node("UPLOAD", width=200, tooltip=_tt("x3dh_step_node_upload")),
            ft.Text("↓", size=24),
            shared_steps.var_node("Server state", "alice_bundle + OPK map saved", width=320, height=95),
        ],
        spacing=8,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    )
    ...
    step3 = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    shared_steps.state_panel("Alice local (after)", _alice_local_rows(after_state.get("alice_local"))),
                    shared_steps.state_panel("Server (after)", _server_alice_rows(after_server)),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=20,
                wrap=True,
...
def _build_upload_new_opk_steps(before_state: dict, after_state: dict) -> list[dict[str, Any]]:
    before_server = before_state.get("server_state") if isinstance(before_state.get("server_state"), dict) else {}
    after_server = after_state.get("server_state") if isinstance(after_state.get("server_state"), dict) else {}
    ...
    step2 = ft.Column(
        controls=[
            ...
            shared_steps.func_node(
                "GENERATE_DH",
                width=200,
                tooltip=_tt("x3dh_step_node_generate_dh"),
            ),
            ft.Text("↓", size=24),
            ft.Row(
                controls=[
            ...
            shared_steps.var_node("Assigned ID", str(new_opk_id), width=220),
        ],
        spacing=8,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    )
    ...
    step3 = ft.Column(
        controls=[
            ...
            shared_steps.var_node("Alice OPK id", str(new_opk_id), width=240),
            ft.Text("↓", size=24),
            shared_steps.func_node("UPLOAD OPK", width=220, tooltip=_tt("x3dh_step_node_upload_opk")),
            ft.Text("↓", size=24),
            shared_steps.var_node("Server OPK set", "alice_available_opk_ids + alice_opk_public_by_id", width=360, height=95),
        ],
        spacing=8,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    )
    ...
    step4 = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    shared_steps.state_panel("Alice local (after)", _alice_local_rows(after_state.get("alice_local")), highlight_labels={"OPK_count_local"}),
                    shared_steps.state_panel("Server (after)", _server_alice_rows(after_server), highlight_labels={"OPK_count_server", "OPK_ids_server"}),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=20,
                wrap=True,
...
def _build_upload_new_spk_steps(before_state: dict, after_state: dict) -> list[dict[str, Any]]:
    before_server = before_state.get("server_state") if isinstance(before_state.get("server_state"), dict) else {}
    after_server = after_state.get("server_state") if isinstance(after_state.get("server_state"), dict) else {}
    before_alice = before_state.get("alice_local") if isinstance(before_state.get("alice_local"), dict) else {}
    ...
    step2 = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    shared_steps.var_node("SPK_pub", shared_steps.last_key_chars(spk_pub), width=240, full_value=spk_pub, tooltip=_tt("x3dh_step_key_spk_pub")),
                    shared_steps.var_node("SPK_priv", shared_steps.last_key_chars(spk_priv), width=240, full_value=spk_priv, tooltip=_tt("x3dh_step_key_spk_priv")),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=20,
            ),
    ...
    step3 = ft.Column(
        controls=[
            ...
            ft.Text("↓", size=24),
            shared_steps.func_node("SIGN", width=180, tooltip=_tt("x3dh_step_node_sign")),
            ft.Text("↓", size=24),
            shared_steps.var_node(
                "new SPK signature",
                shared_steps.last_key_chars(new_signature),
                width=380,
                full_value=new_signature,
                tooltip=_tt("x3dh_step_key_spk_sig"),
            ),
        ],
        spacing=8,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    ...
    step4 = ft.Column(
        controls=[
            ...
            shared_steps.var_node("Local SPK", shared_steps.last_key_chars(spk_pub), width=240, full_value=spk_pub, tooltip=_tt("x3dh_step_key_spk_pub")),
            ft.Text("↓", size=24),
            shared_steps.func_node("UPLOAD SPK", width=220, tooltip=_tt("x3dh_step_node_upload_spk")),
            ft.Text("↓", size=24),
            shared_steps.var_node("Server bundle", "signed_prekey_public + signature replaced", width=380, height=95),
        ],
        spacing=8,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    )
    ...
    step5 = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    shared_steps.state_panel("Alice local (after)", _alice_local_rows(after_state.get("alice_local")), highlight_labels={"SPK_pub", "SPK_priv", "SPK_sig"}),
                    shared_steps.state_panel("Server (after)", _server_alice_rows(after_server), highlight_labels={"SPK_pub(server)", "SPK_sig(server)"}),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                spacing=20,
                wrap=True,
...
def _build_custom_steps(
    ...
    if action_name == "generate_alice_registration_material":
        return _build_generate_alice_keys_steps(before_state, after_state)
    if action_name == "upload_alice_initial_bundle":
        return _build_upload_initial_bundle_steps(before_state, after_state)
    if action_name == "alice_uploads_new_opk":
        return _build_upload_new_opk_steps(before_state, after_state)
    if action_name == "alice_rotates_signed_prekey_bundle":
        return _build_upload_new_spk_steps(before_state, after_state)
    if action_name == "request_bob_bundle_for_alice":
        return _build_request_bob_bundle_steps(before_state, after_state)
    if action_name == "alice_verifies_bundle_signature":
        return _build_verify_signature_steps(before_state, after_state)
    if action_name == "alice_generates_ek_and_derives_sk":
        return _build_generate_ek_and_sk_steps(before_state, after_state)
    if action_name == "alice_calculates_associated_data":
        return _build_calculate_ad_steps(before_state, after_state)
    if action_name == "alice_sends_initial_message":
        return _build_send_initial_message_steps(before_state, after_state, action_context or {})
    if action_name == "bob_receives_and_verifies":
        return _build_bob_receives_steps(before_state, after_state)
    return None
...
def show_x3dh_action_step_visualization_dialog(
    ...
    custom_steps = _build_custom_steps(action_name, before_state, after_state, action_context)
    if custom_steps is None:
        return

    show_step_dialog(page, f"Step-by-step visualization: {_action_title(action_name)}", custom_steps)
