...
def _serialize_pqxdh_state(state: PQXDHState) -> dict:
    return asdict(state)


def _deserialize_pqxdh_state(data: dict) -> PQXDHState:
    return PQXDHState(
...
class PQXDHModule(KeyExchangeBaseModule):
    ...
    def _state_data(self) -> dict:
        return _serialize_pqxdh_state(self.state)
    ...
    def import_state(self, data: dict) -> None:
        if isinstance(data, dict) and data:
            if not data.get("events"):
                data["events"] = []
            self.state = _deserialize_pqxdh_state(data)
        else:
            self.state = self._new_state()
